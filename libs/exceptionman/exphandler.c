...
#define SCR_PRINTF(x...) scr_printf(x)
#define PRINTF(x...) printf(x)
#define EE_PRINTF(x...) 
 #define MULTIPRINTF(x...) \
    SCR_PRINTF(x); \
    PRINTF(x); \
    EE_PRINTF(x);
 ////////////////////////////////////////////////////////////////////////
 ...
 void pkoDebug(int cause, int badvaddr, int status, int epc, eeReg *regs)
 {
     ...
     MULTIPRINTF("      Cause %08X  BadVAddr %08X  Status %08X  EPC %08X\n\n",
                cause, badvaddr, status, epc);
 
     for (i = 0; i < 32 / 2; i++) {
        MULTIPRINTF("%4s: %016lX-%016lX %4s: %016lX-%016lX\n",
